...
function wc_tweaks_add_admin_menu()
{
    add_submenu_page(
        'woocommerce',
        __('WC Tweaks', 'woocommerce-tweaks'),
        __('WC Tweaks', 'woocommerce-tweaks'),
        'manage_options',
        'wc-tweaks-settings',
...
function wc_tweaks_settings_init()
{
    ...
    if ('financial' === $active_tab) {
        ...
        add_settings_field(
            ...
            __('Desconto Pix (%)', 'woocommerce-tweaks'),
        ...
        add_settings_field(
            ...
            __('Gateways Prioritários (IDs)', 'woocommerce-tweaks'),
        ...
        add_settings_section(
            ...
            __('Status do Pedido', 'woocommerce-tweaks'),
        ...
        add_settings_field(
            ...
            __('Status Personalizados (slug|Label)', 'woocommerce-tweaks'),
...
{
    echo '<p>' . __('Configure as opções financeiras e de descontos.', 'woocommerce-tweaks') . '</p>';
}

function wc_tweaks_status_section_callback()
{
    echo '<p>' . __('Gerencie os status personalizados de pedido.', 'woocommerce-tweaks') . '</p>';
}
...
function wc_tweaks_pix_discount_callback()
{
    ...
    echo '<p class="description">' . __('Percentual de desconto para o método Asaas Pix.', 'woocommerce-tweaks') . '</p>';
}
...
function wc_tweaks_priority_gateways_callback()
{
    ...
    echo '<p class="description">' . __('IDs dos gateways separados por vírgula para exibir primeiro.', 'woocommerce-tweaks') . '</p>';
}
...
function wc_tweaks_custom_statuses_callback()
{
    ...
    echo '<p class="description">' . __('Um por linha. Formato: slug|Rótulo', 'woocommerce-tweaks') . '</p>';
}
    ...
    <div class="wrap">
        ...
        <nav class="nav-tab-wrapper">
            <a href="?page=wc-tweaks-settings&tab=financial"
                class="nav-tab <?php echo 'financial' === $active_tab ? 'nav-tab-active' : ''; ?>"><?php esc_html_e('Financeiro', 'woocommerce-tweaks'); ?></a>
            <a href="?page=wc-tweaks-settings&tab=statuses"
                class="nav-tab <?php echo 'statuses' === $active_tab ? 'nav-tab-active' : ''; ?>"><?php esc_html_e('Status do Pedido', 'woocommerce-tweaks'); ?></a>
        </nav>
        ...
        <form action="options.php" method="post">
            ...
            submit_button(__('Salvar Configurações', 'woocommerce-tweaks'));
            ?>
        </form>
    </div>
    <?php
